...
from npu_model.configs.programs.parameterized_fused_quant_matmul import (
    make_fused_quant_matmul_instructions,
)
...
from npu_model.configs.programs.parameterized_matmul_i8 import (
    make_matmul_instructions as make_matmul_i8_instructions,
)
...
from npu_model.configs.programs.parameterized_type_conversion import (
    make_type_conversion_instructions,
)
...
# ── matmul_i8 ─────────────────────────────────────────────────────────────────
# No dedicated MLIR directory; int8 matmul reuses fp8 matmul ISA.
# Test with a representative subset of the fp8 shapes.
# make_matmul_i8_instructions = make_matmul_instructions (same generator)
...
def test_matmul_i8_mlir_shape(variant, M, K, N):
    _check(make_matmul_i8_instructions(M, K, N, dram_a=0, dram_b=0, dram_c=0))


# ── fused_quant_matmul (bf16→fp8 + matmul) ────────────────────────────────────
# No dedicated MLIR; corresponds to the fuse of quantize(bf16 A) with fp8 matmul.
# Shapes derived from the 2D quantized_matmul_fp8 variants.
# make_fused_quant_matmul_instructions(M, K, N, dram_a, dram_b, dram_c)
...
def test_fused_quant_matmul_mlir_shape(variant, M, K, N):
    _check(
        make_fused_quant_matmul_instructions(M, K, N, dram_a=0, dram_b=0, dram_c=0)
    )
...
# ── type_conversion ───────────────────────────────────────────────────────────
# SaturnNPU/kernels/type_conversion/ — 43 variants; bf16↔bf16 and bf16↔fp8
# 2D shapes selected.  Non-bf16 dtypes (i1, i64, fp8-only) are excluded since
# the kernel models the bf16 identity/copy path.
# make_type_conversion_instructions(M, N, dram_x, dram_out)


@pytest.mark.parametrize(
    "variant,M,N",
    [
        ("1024x768_bf16", 1024, 768),
        ("1024x3072_fp8", 1024, 3072),
        ("1024x768_fp8", 1024, 768),
        ("241x2560_fp8", _p32(241), 2560),
        ("241x320_fp8", _p32(241), 320),
        ("241x960_fp8", _p32(241), 960),
        ("50x1440_fp8", _p32(50), _p32(1440)),
        ("50x2048_fp8", _p32(50), 2048),
        ("50x32_bf16", _p32(50), 32),
        ("50x32_fp8", _p32(50), 32),
        ("50x720_bf16", _p32(50), _p32(720)),
        ("50x720_fp8", _p32(50), _p32(720)),
        ("50x960_fp8", _p32(50), 960),
        ("64x960_bf16", 64, 960),
    ],
)
def test_type_conversion_mlir_shape(variant, M, N):
    _check(make_type_conversion_instructions(M, N, dram_x=0, dram_out=0))
