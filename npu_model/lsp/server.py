...
from linter import Severity, lint_text
...
def completions(
    ...
    try:
        ...
        for mnemonic, instr_cls in sorted(IsaSpec.operations.items()):
            p = getattr(instr_cls, "params", [])
            detail = ", ".join(p) if p else "no operands"
            items.append(
...
def signature_help(
    ...
    try:
        ...
        if mnemonic in _PSEUDO_PARAMS:
            ...
            operand_params = getattr(IsaSpec.operations[mnemonic], "params", [])
        else:
            return None
        ...
        active = max(0, min(len(tokens) - 1, len(operand_params) - 1))

        label = f"{mnemonic} {', '.join(operand_params)}"
        sig = types.SignatureInformation(
            label=label,
            parameters=[types.ParameterInformation(label=p) for p in operand_params],
        )
